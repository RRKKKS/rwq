...
try:
	...
	Info = "CaRLoS"	
if ":" not in Info:
	token = input("5939645928:AAFkJe2EpGkW_xyclv1vrUY9KN0tXl4J-60");reqtoken = requests.get(f"https://api.telegram.org/bot{token}/getme").json();req = reqtoken["ok"]
	if req == True:
		id = input("1727322857")
...
SaiF = types.InlineKeyboardButton(text = "- Me ☬ .", url = 'https://t.me/R_R_B0')
channel = types.InlineKeyboardButton(text = "- 𝖬𝖾  .", url = 'https://t.me/R_R_B0')
...
def start(message):
    ...
    mar.add(K)
    mar.add(chek)
    ph = "https://graph.org/file/7d436504121dc88fed790.jpg"
    if str(id) == own_id:
    	bot.send_video(message.chat.id,ph,caption=f"""*- hello sir , {message.chat.first_name} .*""",parse_mode="markdown",reply_markup=mar)
